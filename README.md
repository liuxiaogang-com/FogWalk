...
- 单独的橙色轨迹覆盖层；
- 正常/省电两种出行记录模式；Core Motion 与位置位移判断；后台定位配置和 SQLite WAL 增量保存；支持原备份与新记录合并导出；
...
- 已取得路线的结果按真实 ETA 筛选单程预算；无路线时显示估算时间、直线距离和“路线待确认”；
- 手动选点先在 App 内预览路线；外部 Apple 地图导航作为详情中的明确操作；
